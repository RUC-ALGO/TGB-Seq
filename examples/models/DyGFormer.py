...
class DyGFormer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 ...
                 dropout: float = 0.1, max_input_sequence_length: int = 512, device: str = 'cpu', bipartite: bool = False):
        """
        DyGFormer model.
        ...
        self.neighbor_co_occurrence_encoder = NeighborCooccurrenceEncoder(
            neighbor_co_occurrence_feat_dim=self.neighbor_co_occurrence_feat_dim, device=self.device, bipartite=self.bipartite_graph)
    ...
    def get_features(self, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray, padded_nodes_edge_ids: np.ndarray,
        ...
        padded_nodes_neighbor_node_raw_features = self.node_raw_features[torch.from_numpy(
            padded_nodes_neighbor_ids)]
        # Tensor, shape (batch_size, max_seq_length, edge_feat_dim)
        padded_nodes_edge_raw_features = self.edge_raw_features[torch.from_numpy(
            padded_nodes_edge_ids)]
        # Tensor, shape (batch_size, max_seq_length, time_feat_dim)
        padded_nodes_neighbor_time_features = time_encoder(timestamps=torch.from_numpy(
            node_interact_times[:, np.newaxis] - padded_nodes_neighbor_times).float().to(self.device))

        # ndarray, set the time features to all zeros for the padded timestamp
        padded_nodes_neighbor_time_features[torch.from_numpy(
    ...
    def get_patches(self, padded_nodes_neighbor_node_raw_features: torch.Tensor, padded_nodes_edge_raw_features: torch.Tensor,
        ...
        patches_nodes_edge_raw_features = torch.stack(patches_nodes_edge_raw_features, dim=1).reshape(
        ...
        patches_nodes_neighbor_co_occurrence_features = torch.stack(patches_nodes_neighbor_co_occurrence_features, dim=1).reshape(
...
class NeighborCooccurrenceEncoder(nn.Module):
    ...
    def __init__(self, neighbor_co_occurrence_feat_dim: int, device: str = 'cpu', bipartite=False):
        """
    ...
    def count_nodes_appearances(self, src_padded_nodes_neighbor_ids: np.ndarray, dst_padded_nodes_neighbor_ids: np.ndarray):
        ...
        if self.bipartite_graph:
            src_padded_nodes_neighbor_ids = torch.from_numpy(src_padded_nodes_neighbor_ids).to(
                self.device)
            dst_padded_nodes_neighbor_ids = torch.from_numpy(dst_padded_nodes_neighbor_ids).to(
                self.device)
            ...
            for src_row in src_padded_nodes_neighbor_ids:
                unique, reverse, counts = torch.unique(
                    src_row, return_inverse=True, return_counts=True)
                src_unique.append(unique)
                src_reverse.append(reverse)
                src_counts.append(counts)
            src_unique_rows = torch.nn.utils.rnn.pad_sequence(
                src_unique, batch_first=True)
            src_inverse_rows = torch.nn.utils.rnn.pad_sequence(
                src_reverse, batch_first=True)
            src_counts_rows = torch.nn.utils.rnn.pad_sequence(
            ...
            dst_unique_rows = torch.nn.utils.rnn.pad_sequence(
                dst_unique, batch_first=True)
            dst_inverse_rows = torch.nn.utils.rnn.pad_sequence(
                dst_reverse, batch_first=True)
            dst_counts_rows = torch.nn.utils.rnn.pad_sequence(
            ...
            src_padded_node_neighbor_counts_in_dst, dst_padded_node_neighbor_counts_in_src = torch.zeros_like(
                src_padded_nodes_neighbor_ids, dtype=torch.float32), torch.zeros_like(dst_padded_nodes_neighbor_ids, dtype=torch.float32)
            # src itself
            ...
            src_padded_node_neighbor_counts_in_dst = torch.where(
                src_padded_nodes_neighbor_ids == dst, 1, src_padded_node_neighbor_counts_in_dst)
            dst_padded_node_neighbor_counts_in_src = torch.where(
                dst_padded_nodes_neighbor_ids == src, 1, dst_padded_node_neighbor_counts_in_src)
            src_padded_nodes_appearances = torch.cat([src_padded_node_neighbor_counts_in_src.unsqueeze(
                -1), src_padded_node_neighbor_counts_in_dst.unsqueeze(-1)], dim=-1)
            dst_padded_nodes_appearances = torch.cat([dst_padded_node_neighbor_counts_in_dst.unsqueeze(
            ...
            for src_padded_node_neighbor_ids, dst_padded_node_neighbor_ids in zip(src_padded_nodes_neighbor_ids, dst_padded_nodes_neighbor_ids):
                ...
                src_padded_node_neighbor_counts_in_src = torch.from_numpy(
                    src_counts[src_inverse_indices]).float().to(self.device)
                ...
                dst_padded_node_neighbor_counts_in_dst = torch.from_numpy(
                    dst_counts[dst_inverse_indices]).float().to(self.device)
                ...
                # Tensor, shape (src_max_seq_length, )
                src_padded_node_neighbor_counts_in_dst = torch.from_numpy(src_padded_node_neighbor_ids.copy(
                )).apply_(lambda neighbor_id: dst_mapping_dict.get(neighbor_id, 0.0)).float().to(self.device)
                # Tensor, shape (src_max_seq_length, 2)
                src_padded_nodes_appearances.append(torch.stack(
                ...
                # Tensor, shape (dst_max_seq_length, )
                dst_padded_node_neighbor_counts_in_src = torch.from_numpy(dst_padded_node_neighbor_ids.copy(
                )).apply_(lambda neighbor_id: src_mapping_dict.get(neighbor_id, 0.0)).float().to(self.device)
                # Tensor, shape (dst_max_seq_length, 2)
                dst_padded_nodes_appearances.append(torch.stack(
            ...
            # Tensor, shape (batch_size, src_max_seq_length, 2)
            src_padded_nodes_appearances[torch.from_numpy(
                src_padded_nodes_neighbor_ids == 0)] = 0.0
            # Tensor, shape (batch_size, dst_max_seq_length, 2)
            dst_padded_nodes_appearances[torch.from_numpy(
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        hidden_states = self.linear_layers[1](self.dropout(
            F.gelu(self.linear_layers[0](self.norm_layers[1](outputs)))))
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
