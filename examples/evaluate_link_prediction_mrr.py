...
from evaluate_models_utils_mrr import evaluate_model_link_prediction_multi_negs, evaluate_edge_bank_link_prediction
...
if __name__ == "__main__":
    ...
    node_raw_features, edge_raw_features, full_data, train_data, val_data, test_data, new_node_val_data, new_node_test_data = \
        get_link_prediction_data(dataset_name=args.dataset_name, val_ratio=args.val_ratio, test_ratio=args.test_ratio,dataset_path=args.dataset_path)
    ...
    test_idx_data_loader = get_idx_data_loader(indices_list=list(range(len(test_data.src_node_ids))), batch_size=args.multi_negs_batch_size, shuffle=False)
    ...
    if args.model_name == 'EdgeBank':
        ...
        for run in range(args.num_runs):
            ...
            # args.load_model_name = f'{args.model_name}_seed{args.seed+run}'
            args.load_model_name = f'{args.model_name}_seed{args.seed+run}_batchsize{args.batch_size}_num_neighbors{args.num_neighbors}_dropout{args.dropout}_sample_neighbor_strategy{args.sample_neighbor_strategy}_numlayers{args.num_layers}'

            args.save_result_name = f'{args.model_name}_seed{args.seed+run}'
            ...
            load_model_folder = f"{args.save_model_path}/saved_models/{args.model_name}/{args.dataset_name}/{args.load_model_name}"
            ...
            test_metrics = evaluate_model_link_prediction_multi_negs(model_name=args.model_name,
                                                                       model=model,
                                                                       neighbor_sampler=full_neighbor_sampler,
                                                                       evaluate_idx_data_loader=test_idx_data_loader,
                                                                       evaluate_neg_edge_sampler=test_neg_edge_sampler,
            ...
            #         logger.info(f'validate {metric_name}, {average_val_metric:.8f}')
            #         val_metric_dict[metric_name] = average_val_metric

                # logger.info(f'new node validate loss: {np.mean(new_node_val_losses):.8f}')
                ...
                #     logger.info(f'new node validate {metric_name}, {average_new_node_val_metric:.8f}')
            ...
            # logger.info(f'new node test loss: {np.mean(new_node_test_losses):.8f}')
            ...
            #     logger.info(f'new node test {metric_name}, {average_new_node_test_metric:.8f}')
            ...
            # if args.model_name not in ['JODIE', 'DyRep', 'TGN']:
            #     result_json = {
            #         "validate metrics": {metric_name: f'{val_metric_dict[metric_name]:.8f}' for metric_name in val_metric_dict},
            #         # "new node validate metrics": {metric_name: f'{new_node_val_metric_dict[metric_name]:.8f}' for metric_name in new_node_val_metric_dict},
            #         "test metrics": {metric_name: f'{test_metric_dict[metric_name]:.8f}' for metric_name in test_metric_dict},
            #         # "new node test metrics": {metric_name: f'{new_node_test_metric_dict[metric_name]:.8f}' for metric_name in new_node_test_metric_dict}
            #     }
            # else:
            result_json = {
        ...
        #         # logger.info(f'average validate {metric_name}, {np.mean([val_metric_single_run[metric_name] for val_metric_single_run in val_metric_all_runs]):.8f} '
        #                     f'± {np.std([val_metric_single_run[metric_name] for val_metric_single_run in val_metric_all_runs], ddof=1):.8f}')
            ...
            #     logger.info(f'average new node validate {metric_name}, {np.mean([new_node_val_metric_single_run[metric_name] for new_node_val_metric_single_run in new_node_val_metric_all_runs]):.8f} '
            #                 f'± {np.std([new_node_val_metric_single_run[metric_name] for new_node_val_metric_single_run in new_node_val_metric_all_runs], ddof=1):.8f}')
        ...
        #     logger.info(f'average new node test {metric_name}, {np.mean([new_node_test_metric_single_run[metric_name] for new_node_test_metric_single_run in new_node_test_metric_all_runs]):.8f} '
        #                 f'± {np.std([new_node_test_metric_single_run[metric_name] for new_node_test_metric_single_run in new_node_test_metric_all_runs], ddof=1):.8f}')
